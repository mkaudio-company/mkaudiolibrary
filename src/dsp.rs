...
impl Convolution {
    ...
    pub fn process(&self, input: f64) -> f64 {
        ...
        guard.push(input);
        crate::simd::dot(&guard, &self.kernel)
    }
        ...
        unsafe {
            no_denormals(|| {
                for index in 0..input_guard.len().min(output_guard.len()) {
                    ...
                    output_guard[index] = crate::simd::dot(&buffer_guard, &self.kernel);
                }
            });
        }
    }
...
impl Component for Resistor {
    ...
    fn get_conductance(&self, _dt: f64) -> f64 {
        self.conductance
    }
    fn get_current_source(&self, _dt: f64) -> f64 {
        0.0
    }
...
impl Component for Capacitor {
    ...
    fn update_state(&mut self, v_a: f64, v_b: f64, _dt: f64) {
...
impl Limit {
    ...
    pub fn run(&mut self, input: &Buffer<f64>, output: &Buffer<f64>) {
        ...
        unsafe {
            no_denormals(|| {
                for index in 0..len {
                    let amplified = input_guard[index] * gain_linear;
                    let abs_sample = amplified.abs();
...
pub struct Delay {
    ...
    // `run()` so the audio thread never allocates once warmed up. Guarded
    // by a `Mutex` (rather than a `&mut self` receiver) to preserve the
    // existing thread-safe, shared-access API of `Delay`.
