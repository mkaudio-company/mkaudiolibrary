...
pub fn window(kind: WindowFunction, len: usize) -> Vec<f64> {
    if len == 0 {
        return Vec::new();
    }
    if len == 1 {
...
pub fn stft(signal: &[f64], config: &StftConfig) -> Vec<Vec<Complex64>> {
    ...
    (0..num_frames)
        .map(|frame| {
            ...
            let windowed: Vec<Complex64> = (0..config.fft_size)
                .map(|i| Complex64::new(signal[start + i] * win[i], 0.0))
                .collect();
            fft(&windowed)
        })
        .collect()
...
pub fn istft(frames: &[Vec<Complex64>], config: &StftConfig) -> Vec<f64> {
...
pub fn mr_stft(signal: &[f64], configs: &[StftConfig]) -> Vec<Vec<Vec<Complex64>>> {
...
mod tests {
    ...
    fn istft_reconstructs_signal_with_hann_50_percent_overlap() {
        ...
        for i in skip..(signal.len() - skip).min(reconstructed.len()) {
            assert!(
                (signal[i] - reconstructed[i]).abs() < 1e-6,
                "sample {}: {} vs {}",
                i,
                signal[i],
                reconstructed[i]
