...
impl WavAudioFormat {
    fn from_num(num: usize) -> Option<Self> {
        if num == 0x0001 {
            ...
            return Some(Self::IEEEFloat);
        } else if num == 0x0006 {
            return Some(Self::ALaw);
        } else if num == 0x0007 {
            return Some(Self::MULaw);
        } else if num == 0xFFFE {
            return Some(Self::Extensible);
        }
        None
    }
...
impl AudioFile {
    ...
    pub fn load(&mut self, path: &str) {
        if let Ok(mut file) = std::fs::File::open(path) {
            let mut buffer = vec![];
            if let Err(error) = std::io::Read::read_to_end(&mut file, &mut buffer) {
                eprintln!("{}", error);
            } else {
    ...
    pub fn set_channels(&mut self, count: usize) {
        self.audio_buffer
            .resize(count, vec![0.0; self.num_sample()]);
    }
    ...
    pub fn set_samples(&mut self, count: usize) {
        for buffer in &mut self.audio_buffer {
    ...
    pub fn set_bit_depth(&mut self, bit_depth: usize) {
    ...
    pub fn bext_mut(&mut self) -> Option<&mut BextChunk> {
        self.bext_chunk.as_mut()
    }
    ...
    pub fn set_bext(&mut self, bext: BextChunk) {
        self.bext_chunk = Some(bext);
    }

    /// Remove the BWF broadcast extension chunk.
    pub fn clear_bext(&mut self) {
        self.bext_chunk = None;
    }

    /// Check if this file has BWF metadata.
    pub fn is_bwf(&self) -> bool {
        self.bext_chunk.is_some()
    }
    ...
    pub fn set_tempo_with_time_sig_at(
        ...
        self.tempo = Some(TempoInfo::with_time_signature_at(
            bpm,
            numerator,
            denominator,
            position,
    ...
    fn read_wav(&mut self, buffer: &[u8]) {
        ...
        let _format_chunk_id =
            String::from_utf8(buffer[index_of_format_chunk..index_of_format_chunk + 4].to_vec());
        let _format_chunk_size =
            get_u32(buffer, index_of_format_chunk + 4, Endianness::Little) as usize;
        let audio_format =
            WavAudioFormat::from_num(
                get_u16(buffer, index_of_format_chunk + 8, Endianness::Little) as usize,
            );
        ...
        let num_bytes_per_second =
            get_u32(buffer, index_of_format_chunk + 16, Endianness::Little) as usize;
        let num_bytes_per_block =
        ...
        if !(1..=128).contains(&num_channels) {
            eprintln!(
                "ERROR: this WAV file seems to be an invalid number of channels (or corrupted?)"
            );
            return;
        }
        if num_bytes_per_second != num_channels * self.sample_rate * self.bit_depth / 8
            || num_bytes_per_block != num_channels * num_bytes_per_second
        {
        ...
        let _data_chunk_id =
            String::from_utf8(buffer[index_of_data_chunk..index_of_data_chunk + 4].to_vec());
        ...
        for index in 0..num_samples {
            for channel in 0..num_channels {
                let sample_index = samples_start_index
                    + num_bytes_per_block * index
                    + channel * num_bytes_per_sample;

                if sample_index + (self.bit_depth / 8) > buffer.len() {
                    eprintln!(
                ...
                if self.bit_depth == 8 {
                    ...
                    let mut sample = (((buffer[sample_index + 2] as u32) << 16)
                        | ((buffer[sample_index + 1] as u32) << 8)
                        | buffer[sample_index] as u32)
                        .cast_signed();
        ...
        if index_of_xmlchunk > 0 {
            ...
            if let Ok(chunk) = String::from_utf8(
                buffer[index_of_xmlchunk + 8..index_of_xmlchunk + 8 + chunk_size].to_vec(),
            ) {
    ...
    fn read_aiff(&mut self, buffer: &[u8]) {
        ...
        if index_of_sound_data_chunk == 0
            || index_of_comm_chunk == 0
            || audio_format == AIFFAudioFormat::Error
        {
        ...
        let _comm_chunk_id =
            String::from_utf8(buffer[index_of_comm_chunk..index_of_comm_chunk + 4].to_vec());
        ...
        let num_samples_per_channel =
            get_u32(buffer, index_of_comm_chunk + 10, Endianness::Big) as usize;
        ...
        let _sound_data_chunk_id = String::from_utf8(
            buffer[index_of_sound_data_chunk..index_of_sound_data_chunk + 4].to_vec(),
        );
        let sound_data_chunk_size =
            get_u32(buffer, index_of_sound_data_chunk + 4, Endianness::Big) as usize;
        ...
        if sound_data_chunk_size - 8 != total_num_audio_sample_bytes
            || total_num_audio_sample_bytes > buffer.len() - samples_start_index
        {
            ...
            for channel in 0..num_channels {
                ...
                if self.bit_depth == 8 {
                    ...
                    let mut sample = ((buffer[sample_index] as i32) << 16)
                        | ((buffer[sample_index + 1] as i32) << 8)
                        | buffer[sample_index + 2] as i32;
    ...
    fn save_wav_internal(&self, path: &str, include_bwf: bool) {
        ...
        let bext_chunk_size = if include_bwf && let Some(bext) = self.bext_chunk.as_ref() {
        ...
        let (cue_chunk_size, list_chunk_size) = if !self.markers.is_empty() {
            ...
            for marker in &self.markers {
                if !marker.label.is_empty() {
                    ...
                    let padded_len = if label_len % 2 == 1 {
                        label_len + 1
                    } else {
                        label_len
                    };
        ...
        if bext_chunk_size > 0 {
            file_size_in_bytes += 8 + bext_chunk_size;
        }
        if cue_chunk_size > 0 {
            file_size_in_bytes += 8 + cue_chunk_size;
        }
        if list_chunk_size > 0 {
            file_size_in_bytes += 8 + list_chunk_size;
        }
        if acid_chunk_size > 0 {
            file_size_in_bytes += 8 + acid_chunk_size;
        }
        ...
        set_u32(
            &mut buffer,
            (self.num_channel() * self.sample_rate * self.bit_depth / 8) as u32,
            Endianness::Little,
        );
        set_u16(
            &mut buffer,
            (self.num_channel() * (self.bit_depth / 8)) as u16,
            Endianness::Little,
        );
        ...
        if let Ok(mut file) = std::fs::File::create(path) {
            if let Err(error) = std::io::Write::write(&mut file, &buffer) {
                eprintln!(
                    "ERROR: couldn't save file to {} from error : {}",
                    path, error
                );
            }
        } else {
            eprintln!("ERROR: couldn't create file to {}", path);
        }
    }
...
fn ten_byte_match(buffer1: &[u8], start1: usize, buffer2: &[u8], start2: usize) -> bool {
    for index in 0..10 {
        if buffer1[start1 + index] != buffer2[start2 + index] {
            return false;
        }
    }
    true
...
fn get_aiff_sample_rate(buffer: &[u8], start: usize) -> usize {
    for table in &AIFF_SAMPLE_RATE_TABLE {
        if ten_byte_match(buffer, start, &table.1, 0) {
            return table.0;
        }
    }
...
#[inline]
fn set_string(buffer: &mut Vec<u8>, string: &str) {
...
fn get_index_of_chunk(buffer: &[u8], chunk: &str, start: usize, endianness: Endianness) -> usize {
    ...
    while index < buffer.len() - datalen {
        if &buffer[index..index + datalen] == chunk.as_bytes() {
            return index;
        }
        index += datalen;
...
fn get_u32(buffer: &[u8], start: usize, endianness: Endianness) -> u32 {
    if buffer.len() >= (start + 4) {
        return match endianness {
            Endianness::Big => {
                ((buffer[start + 3] as u32) << 24)
                    | ((buffer[start + 2] as u32) << 16)
                    | ((buffer[start + 1] as u32) << 8)
                    | buffer[start] as u32
            }
            Endianness::Little => {
...
fn get_u16(buffer: &[u8], start: usize, endianness: Endianness) -> u16 {
    if buffer.len() >= (start + 2) {
        return match endianness {
            Endianness::Big => ((buffer[start + 1] as u16) << 8) | buffer[start] as u16,
            Endianness::Little => ((buffer[start] as u16) << 8) | buffer[start + 1] as u16,
        };
    }
    eprintln!("ERROR: Insufficient buffer length.");
    0
...
fn get_u64(buffer: &[u8], start: usize, endianness: Endianness) -> u64 {
    if buffer.len() >= (start + 8) {
        return match endianness {
            Endianness::Big => {
                ((buffer[start + 7] as u64) << 56)
                    | ((buffer[start + 6] as u64) << 48)
                    | ((buffer[start + 5] as u64) << 40)
                    | ((buffer[start + 4] as u64) << 32)
                    | ((buffer[start + 3] as u64) << 24)
                    | ((buffer[start + 2] as u64) << 16)
                    | ((buffer[start + 1] as u64) << 8)
                    | buffer[start] as u64
            }
            Endianness::Little => {
                ((buffer[start] as u64) << 56)
                    | ((buffer[start + 1] as u64) << 48)
                    | ((buffer[start + 2] as u64) << 40)
                    | ((buffer[start + 3] as u64) << 32)
                    | ((buffer[start + 4] as u64) << 24)
                    | ((buffer[start + 5] as u64) << 16)
                    | ((buffer[start + 6] as u64) << 8)
                    | buffer[start + 7] as u64
            }
        };
    }
    0
...
fn read_bext_chunk(buffer: &[u8], index: usize) -> BextChunk {
    ...
    if data_start + 412 <= buffer.len() {
        bext.umid
            .copy_from_slice(&buffer[data_start + 348..data_start + 412]);
    }
    ...
    if coding_history_start < buffer.len() {
        ...
        if chunk_end <= buffer.len() {
            bext.coding_history = read_fixed_string(
                buffer,
                coding_history_start,
                chunk_end - coding_history_start,
            );
        }
    }
...
fn write_bext_chunk(buffer: &mut Vec<u8>, bext: &BextChunk) {
    ...
    set_u16(
        buffer,
        bext.max_momentary_loudness as u16,
        Endianness::Little,
    );
    set_u16(
        buffer,
        bext.max_short_term_loudness as u16,
        Endianness::Little,
    );
...
fn write_acid_chunk(
    ...
    set_u16(
        buffer,
        tempo.time_sig_denominator as u16,
        Endianness::Little,
    );
