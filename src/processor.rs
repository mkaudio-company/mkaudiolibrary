...
//!     fn run(& self, input: & Option<Buffer<f64>>, sidechain_in: & Option<Buffer<f64>>, output: & mut Buffer<f64>, sidechain_out: & mut Option<Buffer<f64>>)
//!     {
//!         if input.is_none()
//!         {
//!             return;
...
pub trait Processor
{
    ...
    fn run(& self, input: & Option<Buffer<f64>>, sidechain_in : & Option<Buffer<f64>>,
           output: & mut Buffer<f64>, sidechain_out : & mut Option<Buffer<f64>>);
}
///Loads plugin.
